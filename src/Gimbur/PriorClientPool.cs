...
/// the inference server URL, prior mode, and (for placement) the action serializer's
/// topology. Reusing clients across <c>/choose-action</c> requests keeps the
...
public static class PriorClientPool
{
    ...
    /// </summary>
    public static PriorClient Get(
        string nnUrl,
        PriorMode mode,
        PlacementActionSerializer? actionSerializer)
    {
        var topologyKey = actionSerializer is null ? "<none>" : actionSerializer.TopologyKey;
        var key = $"{nnUrl}|{mode}|{topologyKey}";

        return _clients.GetOrAdd(key, _ => new PriorClient(
            nnUrl,
            mode,
            actionSerializer,
            pooled: true));
