...
public sealed class CatanState : ICoreState
{
    ...
    public string SerializeHumanReadable()
    {
        ...
        // Section 2: Robber
        sb.Append('|');
        sb.Append(CrockfordBase32.Encode(Board.RobberTile));

        // Section 3: Current Turn (player + stage, concatenated)
        sb.Append('|');
        sb.Append(CrockfordBase32.Encode(CurrentPlayer));
        sb.Append(CrockfordBase32.Encode((int)Stage));

        // Section 4: Longest Road / Largest Army (concatenated)
        sb.Append('|');
        sb.Append(CrockfordBase32.Encode(LongestRoadOwner));
        sb.Append(CrockfordBase32.Encode(LargestArmyOwner));

        // Section 5: Vertices (concatenated)
        sb.Append('|');
        for (var vi = 0; vi < Board.Topology.VertexCount; vi++)
        {
            sb.Append(CrockfordBase32.Encode(Board.VertexOccupancy[vi].ToToken()));
        }
        ...
        {
            sb.Append(CrockfordBase32.Encode(Board.EdgeOccupancy[ei].ToToken()));
        }
        ...
        {
            sb.Append(CrockfordBase32.Encode((int)Board.PortType(pi)));
        }
        ...
        for (var player = 1; player <= PlayerCount; player++)
        {
            ...
            sb.Append(CrockfordBase32.Encode(_resources[player, ResourceToIndex(ResourceType.Wood)]));
            sb.Append(CrockfordBase32.Encode(_resources[player, ResourceToIndex(ResourceType.Brick)]));
            sb.Append(CrockfordBase32.Encode(_resources[player, ResourceToIndex(ResourceType.Sheep)]));
            sb.Append(CrockfordBase32.Encode(_resources[player, ResourceToIndex(ResourceType.Wheat)]));
            sb.Append(CrockfordBase32.Encode(_resources[player, ResourceToIndex(ResourceType.Ore)]));
        }
        ...
        for (var player = 1; player <= PlayerCount; player++)
        {
            ...
            sb.Append(CrockfordBase32.Encode(_knightsPlayed[player]));
        }
        ...
        for (var player = 1; player <= PlayerCount; player++)
        {
            ...
            {
                sb.Append(CrockfordBase32.Encode(_devCards[player, card]));
            }
        }
        ...
        string serialized)
    {
        ...
        for (var player = 1; player <= playerCount; player++)
        {
            ...
            resources[player, ResourceToIndex(ResourceType.Wood)] = CrockfordBase32.Decode(group[0]);
            resources[player, ResourceToIndex(ResourceType.Brick)] = CrockfordBase32.Decode(group[1]);
            resources[player, ResourceToIndex(ResourceType.Sheep)] = CrockfordBase32.Decode(group[2]);
            resources[player, ResourceToIndex(ResourceType.Wheat)] = CrockfordBase32.Decode(group[3]);
            resources[player, ResourceToIndex(ResourceType.Ore)] = CrockfordBase32.Decode(group[4]);
        }
    ...
    {
        var humanReadable = SerializeHumanReadable();
