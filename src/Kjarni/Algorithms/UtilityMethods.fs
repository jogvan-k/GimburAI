module Kjarni.Algorithms.UtilityMethods
...
let toRemainingSearch (depth: searchLimit) tn =
    match depth with
    | Ply (plies, timeLimit) -> Plies plies, timeLimit
    | Turn (turns, timeLimit) -> Turns(tn + turns, 0), timeLimit
...
let isDepthReached current target =
    ...
    | Plies (plies = remaining;) ->
        match target with
        | Plies (plies = r;) -> remaining >= r
