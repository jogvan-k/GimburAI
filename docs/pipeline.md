...
The model emits raw logits. The current composite settlement-road export is temporarily aggregated by the Python loader into settlement-vertex targets; it is not a model or tokenizer vocabulary. `policyTargetTemperature` transforms positive legal visit shares by exponent `1 / temperature` and renormalizes them. Serving softmaxes the fixed-width stage policy; stage-specific masking remains external.
